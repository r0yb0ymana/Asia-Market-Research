...
```mermaid
flowchart LR
    A[Employer / insurer contract] --> B[Employee eligibility + onboarding]
    B --> C[Health screening / HRA]
    C --> D{Risk stratification}
    D -->|Chronic risk| E[16-week programme: diabetes / weight / hypertension]
    D -->|Mental health| F[EAP: counselling, therapy sessions]
    E --> G[Multidisciplinary coach team via async in-app chat + AI journals]
    F --> G
    G --> H[24/7 careline / WhatsApp textline escalation]
    G --> I[End-of-programme screening]
    I --> J[Outcomes + ROI report to payor]
```
