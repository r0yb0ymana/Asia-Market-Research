...
```mermaid
flowchart LR
    subgraph CAT["Category expansion"]
        W[Weight/GLP-1<br/>proven now] --> L[Longevity membership<br/>ARPU apex]
        W --> C[Chronic metabolic<br/>employer-funded]
        L --> G[Women's / Men's health]
    end
    subgraph GEO["Geographic expansion"]
        MY[Malaysia<br/>volume + proof] --> SG[Singapore<br/>ARPU + credibility]
        SG --> HK[Hong Kong<br/>max margin]
    end
    W -.same spine.-> MY
    L -.same spine.-> SG
```
...
```mermaid
flowchart TB
    A[Active patients by market/segment] --> D[Revenue]
    B[ARPU by tier/market] --> D
    C[12-mo retention] --> A
    C --> E[LTV per enrolled patient]
    F[CAC by channel] --> G[Contribution after CAC]
    D --> G
    E --> G
    H[Mix shift MY→SG→HK] --> I[Blended gross margin]
    D --> I
    G --> J[Path to profitability]
    I --> J
```
