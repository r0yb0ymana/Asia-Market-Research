...
| **Medimind KL** (WellTech clinic) | Provides the **regulated clinical act** — its doctors run the telehealth consults, prescribe, and **dispense from its own pharmacy** — for a **fair-market fee-for-service**. |

**Commercial mechanic (drug economics):** the programme is priced **all-in (RM999/month, drug included)** and **NewCo collects the full patient payment**. Medimind bills NewCo internally for (a) a **doctor service fee** per consult and (b) **drug-at-cost + a dispensing fee**. **NewCo keeps the spread** (programme margin *and* drug margin) — without holding a pharmacy or wholesale licence, because it rides Medimind's existing pharmacy. **NewCo-as-drug-wholesaler is deferred to Phase 2** (a direct-sourcing margin lever that requires a Type A wholesale poisons licence + pharmacist + GDP cold-chain, and only pays off at multi-clinic volume).
...
- **Drug supply economics.** Confirm Medimind's GLP-1 procurement cost early (the model flags this as unvalidated) — it sets your real margin, and it's now a quick internal check rather than an external negotiation.
