...
```mermaid
timeline
    title Welltech regional entry sequence — one operating system, three deployments
    Malaysia (Market 1) : Prove WhatsApp clinical-ops + AI machine : Validate cash-pay GLP-1 demand : Beachhead Klang Valley : Publish first outcome cohort
    Singapore (Market 2) : Bank HCSA licence + CGO governance : Employer metabolic beachhead vs 12-17% trend : Publish outcomes to NOVI standard : Capital + credibility hub
    Hong Kong (Market 3) : Deploy proven stack at max ARPU : Dual wedge - GLP-1 programme + HNW longevity : No new regulatory build : GBA / mainland-shopper option
```
