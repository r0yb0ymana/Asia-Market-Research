...
flowchart LR
    subgraph Demand
        A[Employer/Insurer] -->|premium/PEPM| B[TPA/MCO]
        C[Cash-pay consumer]
    end
    subgraph Delivery
        B -->|panel, capped fees, 3-6mo payment| D[GP clinic]
        C --> D
        C --> E[Pharmacy chain]
        C --> F[Lab/screening chain]
        D -->|referral| G[Private hospital specialist/OPD]
        F -->|abnormal result, unmanaged| G
        G --> H[Admission/procedure]
    end
    subgraph MarginPools
        D -.->|~75% of bill| M1[Drug dispensing margin]
        F -.-> M2[Screening bundle margin]
        G -.-> M3[Specialist fee + HSS markup]
        H -.-> M4[Hospital HSS ~59-70% of bill]
    end
```
