...
```mermaid
flowchart LR
    A[Demand: mass-affluent / expat / HNW / employer] --> B[Distribution: insurers Bupa-Blua, Bowtie, AIA / employer benefits / WhatsApp & referral]
    B --> C[Care orchestration: front desks, WhatsApp threads - fragmented, manual]
    C --> D[Clinical delivery: private GPs, specialists, chains QHMS/EC, hospitals]
    D --> E[Diagnostics: hospital labs, Prenetics/CircleDNA, imaging centres]
    D --> F[Pharmacy & fulfilment: registered pharmacies, clinic dispensing, same-day delivery]
    E --> G[Records: eHealth+ spine, siloed clinic EMRs]
    F --> G
```
...
```mermaid
flowchart TD
    P[Patient WhatsApp] -->|enquiry / check-in| API[WhatsApp Business API gateway]
    API --> BOT[AI triage & templated journeys - Cantonese / English / Putonghua]
    BOT -->|routine| CRM[Care CRM + consent log + audit trail]
    BOT -->|clinical flag| NURSE[Nurse / health-coach queue]
    NURSE -->|escalation| DR[HK-registered doctor - video or in-person]
    DR --> RX[Registered pharmacy dispensing + delivery]
    CRM --> EHR[Clinic EMR → eHealth+ deposit]
    RX --> P
    DR --> P
```
